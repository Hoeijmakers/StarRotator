...
int N;
{
  double Q5,ratio,cneutral,c1ion,c2ion,c3ion,c4ion,mu;
  double D0,Ua,Ub,Na,Nb,theta,psip;
  int i,charge;
  static int l = 2;
  ...
  if(line[N].code < 100.0) {
    ...
    else if(approx(line[N].code,c4ion,0.001) == 1) charge = 4;
    else {
	  printf("Could not determine charge in pop\n");
	  printf("Setting charge = 0\n");
	  charge = 0;
    }
    if(approx(POP[l].atom,line[N].code,0.4) != 1) {
      for(i=2;i<NATOM-NMOL;i++) {
	if(approx(POP[i].atom,line[N].code,0.4) == 1) {
	  l = i;
	  break;
	}
      }
    ...
    for(i=0;i<Ntau;i++) {
	 ...
	 exp(-1.1605e+4*line[N].El/model->T[i])/(Q5*pfunction(V,cneutral,i));
      if(charge == 1) line[N].xnum[i] = line[N].abund*model->NA[i]*
	 POP[l].R1[i]*exp(-1.1605e+4*line[N].El/model->T[i])/
	 (Q5*pfunction(V,c1ion,i));
      if(charge == 2) line[N].xnum[i] = line[N].abund*model->NA[i]*
	 POP[l].R1[i]*POP[l].R2[i]*exp(-1.1605e+4*line[N].El/model->T[i])/
	 (Q5*pfunction(V,c2ion,i));
      if(charge == 3) line[N].xnum[i] = line[N].abund*model->NA[i]*
	 POP[l].R1[i]*POP[l].R2[i]*POP[l].R3[i]*
	 exp(-1.1605e+4*line[N].El/model->T[i])/(Q5*pfunction(V,c3ion,i));
      if(charge == 4) line[N].xnum[i] = line[N].abund*model->NA[i]*
	 POP[l].R1[i]*POP[l].R2[i]*POP[l].R3[i]*POP[l].R4[i]*
	 exp(-1.1605e+4*line[N].El/model->T[i])/(Q5*pfunction(V,c4ion,i));
    }
    return;
  } else {
    mu = line[N].chi2;
...
int flagw;
{
  ...
  for(i=2;i<NATOM-NMOL;i++) {
    ...
    for(j=0;j<Ntau;j++) {
      ...
      if(atom[i].maxcharge == 4) {
         R5 = (4.830e+15/model->Ne[j])*(pfunction(V,POP[i].atom+0.5,j)/
	 pfunction(V,POP[i].atom+0.4,j))*T15*
	 exp(-1.16e+4*atom[i].I5/model->T[j]);
	 Q1 += R5;
      }
      if(atom[i].maxcharge >= 3)  {
	 POP[i].R4[j] = (4.830e+15/model->Ne[j])*(pfunction(V,POP[i].atom+0.4,j)/
