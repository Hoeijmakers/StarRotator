...
main(int argc, char *argv[])
{
  ...
  printf("\nEnter microturbulent velocity (km/s) > ");
  ni = scanf("%lf",&vt);
  vturb = vt*1.0e+05;
  ...
  while(bwline(&wave,line,atom,&ew,qf) == 1) {
     ...
     printf("%8.3f  %5.1f %6.3f  %4.2f  %7.3f  %7.3f  %6.3f\n",line[0].wave,
               line[0].code,line[0].El,vt,Atot,AH,MH);
     fprintf(fp,"%8.3f  %5.1f %6.3f %4.2f  %7.3f  %7.3f  %6.3f\n",
               line[0].wave,line[0].code,line[0].El,vt,Atot,AH,MH);
     fclose(fp);
     fp = fopen(ofile,"a");
  }
