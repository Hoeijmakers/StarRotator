...
main(int argc, char *argv[])
{
  ...
  double scale,res,space,sum1,sum2,a,z,Ds,n1,n2,ss,R;
  double l,d;
  char name[15];
  int ni;

  qsize = 1000000;
  printf("Max number of points = %lu\n",qsize);
  ...
  if(argc != 5) {
    ...
    ggets(ofile);
    printf("\nEnter output resolution R = lambda/delta lambda > ");
    ni = scanf("%lf",&R);
    printf("\nEnter spacing in Angstroms of spectrum in the output file > ");
    ni = scanf("%lf",&space);
  } else {
  ...
  k = 0;
  while(k < qsize-1 && read(fd,&Depth,sizeof(float)) > 0) {
    depth[k] = Depth;
    k++;
  }
